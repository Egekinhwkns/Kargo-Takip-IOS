...
protocol AcceptedCellDelegate: AnyObject {
...
class AcceptedCell: UICollectionViewCell {
    ...
    static let identifier = "AcceptedCell" // kisi tarafindan kabul edildi
    var delegate: AcceptedCellDelegate?
    ...
    func getArray(){
        delegate?.getAccepteds(completion: { kargos in
            self.accepteds = kargos
            self.emptyKargoLabel.isHidden = self.accepteds.isEmpty ? false : true
...
extension AcceptedCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
 
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return accepteds.count
       // return 3
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.statusType = .accepted
        cell.labelTitle.text = accepteds[indexPath.row].company
        cell.kargoDate.text = accepteds[indexPath.row].date
        cell.labelStatus.text = accepteds[indexPath.row].store ?? "(Belirtilmemiş)"
...
//        cell.labelStatus.text = "YTU Store"
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        delegate?.goToSegue(kargo: accepteds[indexPath.row])
